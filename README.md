...
#### What is a share?
A share is a piece of your private key (nsec) that has been split using threshold cryptography (specifically the FROST protocol). Each share alone cannot be used to access your funds or sign transactions. A share is intrinsically tied to its keyset and the other shares within that keyset - it cannot be used with shares from different keysets, and it does not reveal any sensitive information unless combined with other shares from the same keyset (up to the threshold). This threshold-based approach ensures that no single share can compromise your security.

#### What is a group?
A group is a collection of shares that belong to the same keyset. When you create a new keyset, you generate multiple shares that form a group. The group is identified by a unique group key that helps you manage and organize your shares. Each share in a group is mathematically related to the other shares in that specific group - they are designed to work together and cannot be mixed with shares from other groups. This relationship ensures that shares from different keysets cannot be combined to reconstruct a private key. In Igloo, we use the terms "group" and "keyset" interchangeably - a keyset refers to the complete set of shares plus the group public key that identifies them.
